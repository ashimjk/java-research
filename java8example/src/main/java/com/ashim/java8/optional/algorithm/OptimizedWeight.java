...
//			System.out.println((root.right.index + 1) + "st day=eat");
			...
			String index = (root.right.index + 1) + "-" + root.right.data;

			if (root.right.right != null) {
				printRoute(root.right);
			} else if (nodeLookup.get(index) != null) {
				root.right.right = nodeLookup.get(index);
				printRoute(root.right);
			}
...
//			System.out.println((root.left.index + 1) + "st day=not eat");
			...
			String index = (root.left.index + 1) + "-" + root.left.data;


			if (root.left.left != null) {
				printRoute(root.left);
			} else if (nodeLookup.get(index) != null) {
				root.left.left = nodeLookup.get(index);
				printRoute(root.left);
			}
		}
	}

	static Optional<Node<Integer>> getNode(Node<Integer> node) {

		String index = (node.index + 1) + "-" + node.data;
		if (node.left != null && node.right != null) {
			return Optional.of(node);
		} else if (nodeLookup.get(index) != null) {
			return Optional.of(nodeLookup.get(index));
		} else {
	...
	static int minimumWeight(int level, int stressPoint, Node<Integer> node) {
		...
		if (level < N) {
			...
			if (stressPoint == 0) {
				...
				if (left > right) {
					nodeLookup.put(level + "-" + returnValue, nRight);
				} else {
					nodeLookup.put(level + "-" + returnValue, nLeft);
				}
